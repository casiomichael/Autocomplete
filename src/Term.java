...
public class Term implements Comparable<Term> {
	...
	public Term(String word, double weight) {
		...
		if (word == null) throw new NullPointerException("input a word!");
		myWord = word;
		myWeight = weight;
	}
	...
	public static class PrefixOrder implements Comparator<Term> {
		...
		public int compare(Term v, Term w) {
			...
			int lengthV = r;
			int lengthW = r;
			if (v.myWord.length() < r) lengthV = v.myWord.length();
			String vR = v.myWord.substring(0,lengthV);
			if (w.myWord.length() < r) lengthW = w.myWord.length();
			String wR = w.myWord.substring(0,lengthW);
			return vR.compareTo(wR);
		}
	}
	...
	public static class ReverseWeightOrder implements Comparator<Term> {
		public int compare(Term v, Term w) {
			...
			if (v.myWeight == w.myWeight) return 0;
